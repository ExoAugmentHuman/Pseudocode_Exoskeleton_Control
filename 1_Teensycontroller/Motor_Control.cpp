...
Motor_Control::Motor_Control(uint32_t id, int Can_id,double Gear_ratio)
{
  ID = id;
...
//void Motor_Control::receive_CAN_data()
//{
...
void Motor_Control::DataExplanation(CAN_message_t msgR2)
{
  int len = msgR2.len;
  if (len == 8)
  {
...
//          Serial.println("low voltage protection");
      ...
      case 0x9C: //14: read motor status 2 return temperature, iq current(-2048~2048 mapping-32A to 32A), speed (1dps/LSB), encoder (0~16383)
        ...
        speed_value = speed_value/57.3; // rad/second
...
void Motor_Control::write_PID_RAM()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::write_PID_ROM()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
//******4.Read Acceleration******//
void Motor_Control::read_acceleration()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::write_acceleration_RAM()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::read_encoder()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::write_encoder_offset_RAM(uint16_t encoder_Offset )
{
...
void Motor_Control::write_current_position_as_zero_position()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::read_multi_turns_angle()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::read_single_turns_angle()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::clear_motor_angle_command()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::read_motor_status()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::clear_motor_error()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::read_motor_status_2()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::read_motor_status_3()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::close_motor()
{
  msgW.id = 0x140 + ID;
  msgW.ext = 0;
  msgW.len = 8;
...
void Motor_Control::send_current_command(double current)
{
...
void Motor_Control::send_speed_command(double speedvalue)
{
...
void Motor_Control::send_position_command(double angle)
{
...
void Motor_Control::send_position_command_2(double angle, double max_speed)
{
  angle = angle * 100;
  angleControl = (int32_t)angle;
...
//(limited by maximum angle)//
//(limited by maximum current)//
void Motor_Control::send_position_command_3(double angle, uint8_t spinDirection)
{
  angle = angle * 100;
...
//(limited by maximum angle)//
//(limited by maximum current)//
void Motor_Control::send_position_command_4(double angle, double max_speed, uint8_t spinDirection)
{
  angle = angle * 100;
  angleControl = (uint8_t)angle;
...
void Motor_Control::send_multi_motor_current_command(double Motor1_current, double Motor2_current, double Motor3_current, double Motor4_current)
{
